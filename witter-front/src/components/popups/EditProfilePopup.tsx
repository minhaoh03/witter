...
export function EditProfilePopup(props: EditProfilePopupProps) {
    ...
    const handleSubmit = async (e: SyntheticEvent) => {
        ...
        const createFormData = (): void => {
            if(newProfilePictureInput.files![0] === undefined)
                formData.append('profile_picture', newProfilePictureInput.files![0], 'profpic.png')
            ...
            <div className='relative ml-[35%] mt-[7%] rounded-xl bg-black w-[30%] h-[65vh] overflow-y-scroll overflow-x-hidden'>
                ...
                <div className="flex flex-col mx-[5%] w-[90%]">
                    <div className='flex flex-col self-center h-full w-full'>
                        ...
                        <img src={picLink} onClick={chooseFile} alt='profile pic change' className="cursor-pointer self-center shadow m-3 rounded-full border-black border-4 object-cover w-32 h-32 filter hover:brightness-90"/>
                    </div>
                    <div id='email' className='mb-2'>
                        <span className='absolute text-xs mt-1 ml-2 text-gray-500'>Email</span>
                        <input type="email" ref={emailRef} className='outline-none border-[1px] border-gray-400/[0.5] rounded bg-black px-2 pb-2 pt-5 text-sm w-[100%]' defaultValue={user.email} required/>
                    </div>  
                    <div id='name' className='flex flex-row my-2 w-full'>
                        <div id='firstname' className='w-[47.5%] mr-[5%]'>
                            ...
                            <input id="firstname" ref={fnameRef} className='outline-none border-[1px] border-gray-400/[0.5] rounded bg-black px-2 pb-2 pt-5 text-sm w-[100%]' defaultValue={user.first_name} required/>
                        </div>
                        <div id='lastname' className='w-[47.5%]'>
                            <span className='absolute text-xs mt-1 ml-2 text-gray-500'>Last Name</span>
                            <input id="lastname" ref={lnameRef} className='outline-none border-[1px] border-gray-400/[0.5] rounded bg-black px-2 pb-2 pt-5 text-sm w-[100%]' defaultValue={user.last_name} required/>
                        </div>
                    </div>
                    <div>
                        <span className='absolute text-xs mt-1 ml-2 text-gray-500'>Bio</span>
                        <textarea id="bio" ref={bioRef} className='outline-none border-[1px] border-gray-400/[0.5] rounded bg-black px-2 pb-2 pt-5 text-sm w-[100%]' defaultValue={user.bio} required></textarea>
                    </div>
                    <div id='bday' className='mt-1 mb-10'>
                        <span className='absolute text-xs mt-1 ml-2 text-gray-500'>Date of Birth</span>
                        <input type="date" id="bday" ref={bdayRef} className='outline-none border-[1px] border-gray-400/[0.5] rounded bg-black px-2 pb-2 pt-5 text-sm w-[100%]' defaultValue={user.birth_date} required/>
                    </div>
                </div>
            </div>
        </div>
